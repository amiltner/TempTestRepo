...
type t =
  {
    ...
    mutable parse      : bool list option ;
...
and t_node =
  ...
  | Char of Char.t
  | Reduction of (bool list -> bool list) * t

let compare
    (x1:t)
    (x2:t)
  : int =
  Int.compare x1.uid x2.uid

let equal
    (x1:t)
    (x2:t)
  : bool =
  Int.equal x1.uid x2.uid
...
let singleton s = create (Concat (List.rev_map ~f:(Lazy.from_val % char) (String.to_list_rev s)))
...
let reduction
    (f:bool list -> bool list)
    (x:t)
  : t =
  create (Reduction (f,x))

let from_grammar_production
    (g:Grammar.t)
  ...
  let rec from_production
      (p:Grammar.Production.t)
    : t =
    begin match p with
      | Nonterminal i ->
        begin match GM.find !map i with
          | None ->
      ...
      | Or (p1,p2) ->
        let a1 =
          Lazy.from_fun
            (fun () -> reduction (fun xs -> true::xs) (from_production p1))
        in
        let a2 =
          Lazy.from_fun
            (fun () -> reduction (fun xs -> false::xs) (from_production p2))
        in
        union [a1;a2]
    end
  in
...
let from_grammar
    (g:Grammar.t)
  : t =
  from_grammar_production
    g
    (Grammar.get_initial_production g)

let materialize_nullables
    (x:t)
  : unit =
  let changed = ref true in
  let perform_if_p_tag
      (x:t)
      (next_tag:tag)
      (f:unit -> unit)
    : unit =
    if equal_tag x.p_tag next_tag then
      ()
    else
      (x.p_tag <- next_tag;
       f ())
  in
  let rec update_singlestep_nullables
      (next_tag:tag)
      (x:t)
    : unit =
    perform_if_p_tag
      ...
      (fun () ->
         begin match (node x) with
           | Union xs ->
             ...
             if (Option.is_none x.parse) then
               begin match List.find_map ~f:(fun x -> x.parse) xs with
                 | Some bs as bso ->
                   changed := true;
                   x.parse <- bso
                 | None -> ()
             ...
             List.iter
               ~f:(update_singlestep_nullables next_tag)
               xs
           | Concat xs ->
             let xs = List.map ~f:Lazy.force_val xs in
             if (Option.is_none x.parse) then
               if List.for_all ~f:(fun x -> Option.is_some x.parse) xs then
                 (changed := true;
                  let ps =
                    List.concat_map
                      ~f:(fun x -> Option.value_exn x.parse)
                      xs
                  in
                  x.parse <- Some ps);
             List.iter
  ...
  let rec update_p_calced
    ...
    perform_if_p_tag
      ...
      (fun () ->
         begin match (node x) with
           | Union ts ->
             List.iter ~f:((update_p_calced next_tag) % Lazy.force) ts
           | Concat ts ->
             List.iter ~f:((update_p_calced next_tag) % Lazy.force) ts
           | Char c ->
             ()
           | Reduction (f,x') ->
             update_p_calced next_tag x'
         end)
  in
  fold_until_completion
    ~f:(fun tag ->
        ...
        if !changed then
          ...
          Right (update_p_calced new_tag x))
    Red
...
let nullable
  ...
  if x.p_calced then
    ...
    (materialize_nullables x; Option.is_some x.parse)
...
let extract_parse
  ...
  if x.p_calced then
    ...
    (materialize_nullables x; x.parse)
            ...
            let rec concat_disjunction
              ...
              begin match xs with
                ...
                | h::t ->
                  ...
                  let first_deriv = concat (Lazy.from_fun (fun () -> derivative c h)::t) in
                  begin match extract_parse h with
                    | None -> [first_deriv]
                    | Some p ->
                      let nexts =
                        List.map
...
let compact
  ...
  : t =
  let changed = ref true in
  let perform_if_unmaterialized
      (x:t)
      (f:unit -> unit)
    : unit =
    if Option.is_some x.compacted then
      ()
    else
      f ()
  in
  let rec single_step_materialize
      (x:t)
    : unit =
    perform_if_unmaterialized
      x
      (fun () ->
         begin match (node x) with
           | Union xs ->
             ...
             if len = 1 then
               (changed := true; x.compacted <- Some (List.hd_exn xs))
             else
...
let parse
  ...
  in
  parse a (String.to_list s)

let accepts
    (a:t)
    (s:string)
  : bool =
  Option.is_some (parse a s)
