      ...
      | None ->
        ...
        let anses =
          (*begin match Context.destruct_required ctx l p with
            | None ->*) use_context () @ intro_form () @ elim_form () @ intro_recursion ()
            (*| Some ipc ->
              intro_recursion () @ create_destruct ipc @ synth_options (trd3 ipc) destructed next_var p targets size
              end*)
        in
        List.iter
...
let synth
  ...
  let options =
    List.filter
      ~f:(fun c ->
          (*if Converter.nested_fix c then (print_endline "found it"; print_endline (Converter.show c); print_endline (string_of_int (Converter.size c)));*)
          SymbolicEval.covers_all_examples c output_parse_trees)
      options
  in
  List.iter
    ~f:(print_endline % Converter.show)
    options;
  let options_ranks =
    List.filter_map
      ~f:(fun o ->
  ...
  let ans =
    (Option.value_exn
       (List.min_elt
          ...
          options_ranks))
  in
  print_endline (Float.to_string (snd ans));
  fst ans
